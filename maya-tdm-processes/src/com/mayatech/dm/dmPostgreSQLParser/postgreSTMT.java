...
public class postgreSTMT {

	long statement_id=postgreParser.generateStatementId();
	public String statement_type=postgreParser.STMT_TYPE_UNDETECTED;
	...
	postgreSTMTFrom fromBased=null;
	

	postgreSTMT parentStmt=null;
	...
	ArrayList<postgreSTMT> subStatements=new ArrayList<postgreSTMT>();
	
	ArrayList<postgreSTMTWith> withList=new ArrayList<postgreSTMTWith>();
	ArrayList<postgreSTMTSelect> selectList=new ArrayList<postgreSTMTSelect>();
	ArrayList<postgreSTMTFrom> fromList=new ArrayList<postgreSTMTFrom>();
	...
	void parse(ddmClient ddmClient) {
		...
		postgreParser.determineStatementType(query, stmt_type, related_object);
		
		statement_type=stmt_type.toString();
		statement_related_object =related_object.toString();
		
		isRewritable=
				statement_type.equals(postgreParser.POSTGRE_SELECT) ||
				statement_type.equals(postgreParser.POSTGRE_PLSQL_BLOCK) ||
				statement_type.equals(postgreParser.POSTGRE_INSERT_WITH_SELECT) ||
				statement_type.equals(postgreParser.POSTGRE_UPDATE) ||
				statement_type.equals(postgreParser.POSTGRE_CREATE_VIEW) ||
				statement_type.equals(postgreParser.POSTGRE_CREATE_MATERIALIZED_VIEW) ||
				statement_type.equals(postgreParser.POSTGRE_CREATE_TABLE_AS);
	...
	void parseSelectStmt(ddmClient ddmClient) {
		...
		ArrayList<ddmChunk> chunkArr=postgreParser.getChunks(this.query, postgreParser.invisibleChars, false);
	...
	void splitUnionPart() {
		...
		ArrayList<ddmChunk> chunkArr=postgreParser.getChunks(query, postgreParser.invisibleChars, false);
		int pos_union=postgreParser.getNextChunkId(chunkArr, 0, chunkArr.size(), true, false, false, false, postgreParser.UNION_WORDS);
		if (pos_union==-1) return;
		int pos_first_select=postgreParser.getNextChunkId(chunkArr, 0, chunkArr.size(), true, false, false, false, postgreParser.KEYWORD_SELECT);
		if (pos_first_select==-1) 
			pos_first_select=postgreParser.getNextChunkId(chunkArr, 0, chunkArr.size(), false, false, true, false, null);
	...
	void compileWithPart(ddmClient ddmClient) {
		...
		withChunks=postgreParser.getChunksByComma(with_part);

		for (int i=0;i<withChunks.size();i++) {
			ddmChunk chunk=withChunks.get(i);
			
			postgreSTMTWith with=new postgreSTMTWith();
	...
	void compileSelectPart(ddmClient ddmClient) {
		...
		for (int i=0;i<selectChunks.size();i++) {
			...
			postgreSTMTSelect select=new postgreSTMTSelect();
	...
	void extractWithSelectFromChunks(ddmClient ddmClient) {
		...
		int pos_with=postgreParser.getNextChunkId(chunks, 0, chunks.size(), true, false, false, false, postgreParser.KEYWORD_WITH);
				
		int pos_select=postgreParser.getNextChunkId(chunks, 0, chunks.size(), true, false, false, false, postgreParser.KEYWORD_SELECT);
		if (pos_select==-1) {
			hasError=true;
			ddmClient.mydebug(" extractWithSelectFromChunks : '"+postgreParser.KEYWORD_SELECT+"' not found after "+pos_with);
			return;
		}
		...
		if (pos_with>-1) {
			int pos_as=postgreParser.getNextChunkId(chunks, pos_with, chunks.size(), true, false, false, false, postgreParser.KEYWORD_AS);
			if (pos_as>-1) {
		...
		int pos_from=postgreParser.getNextChunkId(chunks, pos_select, chunks.size(), true, false, false, false, postgreParser.KEYWORD_FROM);
		...
		int pos_end_of_from=postgreParser.getNextChunkId(chunks, pos_from, chunks.size(), true, false, false, false, postgreParser.KEYWORD_MODEL);  
		if (pos_end_of_from==-1) pos_end_of_from=postgreParser.getNextChunkId(chunks, pos_from, chunks.size(), true, false, false, false, postgreParser.KEYWORD_WHERE);
		
		if (pos_end_of_from==-1) pos_end_of_from=postgreParser.getNextChunkId(chunks, pos_from, chunks.size(), true, false, false, false, postgreParser.KEYWORD_ORDER);
		if (pos_end_of_from==-1) pos_end_of_from=postgreParser.getNextChunkId(chunks, pos_from, chunks.size(), true, false, false, false, postgreParser.KEYWORD_GROUP);
		if (pos_end_of_from==-1) pos_end_of_from=postgreParser.getNextChunkId(chunks, pos_from, chunks.size(), true, false, false, false, postgreParser.KEYWORD_HAVING);
		if (pos_end_of_from==-1) pos_end_of_from=postgreParser.getNextChunkId(chunks, pos_from, chunks.size(), true, false, false, false, postgreParser.KEYWORD_CONNECT);
		///create or replace view emp_check_opt as select * from hr.employees with check option
		if (pos_end_of_from==-1) pos_end_of_from=postgreParser.getNextChunkId(chunks, pos_from, chunks.size(), true, false, false, false, postgreParser.KEYWORD_WITH);  
		//cursor c1 select  * from hr.employees for update of email
		if (pos_end_of_from==-1) pos_end_of_from=postgreParser.getNextChunkId(chunks, pos_from, chunks.size(), true, false, false, false, postgreParser.KEYWORD_FOR);  
		...
		ArrayList<ddmChunk> intoCheckArr=postgreParser.getChunks(select_part, postgreParser.invisibleChars, false);
		int pos_into=postgreParser.getNextChunkId(intoCheckArr, 0, intoCheckArr.size(), true, false, false, false, postgreParser.KEYWORD_INTO);
		...
		selectChunks.addAll(postgreParser.getChunksByComma(select_part));
		
		int pos_where=postgreParser.getNextChunkId(chunks, pos_from, chunks.size(), true, false, false, false, postgreParser.KEYWORD_WHERE);
		...
		if (pos_where==-1) {
			...
			pos_end_of_where=postgreParser.getNextChunkId(chunks, pos_where, chunks.size(), true, false, false, false, postgreParser.KEYWORD_MODEL);  			
			if (pos_end_of_where==-1) pos_end_of_where=postgreParser.getNextChunkId(chunks, pos_where, chunks.size(), true, false, false, false, postgreParser.KEYWORD_ORDER);
			if (pos_end_of_where==-1) pos_end_of_where=postgreParser.getNextChunkId(chunks, pos_where, chunks.size(), true, false, false, false, postgreParser.KEYWORD_GROUP);
			if (pos_end_of_where==-1) pos_end_of_where=postgreParser.getNextChunkId(chunks, pos_where, chunks.size(), true, false, false, false, postgreParser.KEYWORD_HAVING);
			if (pos_end_of_where==-1) pos_end_of_where=postgreParser.getNextChunkId(chunks, pos_where, chunks.size(), true, false, false, false, postgreParser.KEYWORD_CONNECT);
			///create or replace view emp_check_opt as select * from hr.employees with check option
			if (pos_end_of_where==-1) pos_end_of_where=postgreParser.getNextChunkId(chunks, pos_where, chunks.size(), true, false, false, false, postgreParser.KEYWORD_WITH);  
			//cursor c1 select  * from hr.employees for update of email
			if (pos_end_of_where==-1) pos_end_of_where=postgreParser.getNextChunkId(chunks, pos_where, chunks.size(), true, false, false, false, postgreParser.KEYWORD_FOR);  
	...
	void compileFromPart(ddmClient ddmClient) {
		...
		fromChunks=postgreParser.getChunksForFromPart(from_part);
		
		for (int i=0;i<fromChunks.size();i++) {
			ddmChunk chunk=fromChunks.get(i);
			
			postgreSTMTFrom from=new postgreSTMTFrom();
	...
	//------------------------------------------------------------------------------------
	void printFrom(ddmClient ddmClient, postgreSTMTFrom from, String padding_str) {
	...
	//------------------------------------------------------------------------------------
	String printExpression(ddmClient ddmClient, postgreSTMTExpr expr) {
	...
	//------------------------------------------------------------------------------------
	void printSelect(ddmClient ddmClient, postgreSTMTSelect select, String padding_str) {
	...
	//------------------------------------------------------------------------------------
	void printSTMT(postgreSTMT stmt, ddmClient ddmClient) {
	...
	public String rewriteSingleSQL(
		...
		if (validateQuery) {
			...
			String only_select_part=postgreParser.extractOnlySelectPart(this.query);
			
			ArrayList<String> testCols=postgreParser.getColNamesFromStatement(ddmClient.connParallel, only_select_part,err);
		...
		try {
			...
			if (toMask) {
				...
				postgreParser.discoverSourceColumns(this, outerSTMT, ddmClient);
			...
			for (int i=0;i<this.subStatements.size();i++) {
				postgreSTMT subStmt=this.subStatements.get(i);
				...
				for (int w=0;w<subStmt.withList.size();w++) {
					postgreSTMTWith with=subStmt.withList.get(w);
				...
				for (int e=0;e<subStmt.selectList.size();e++) {
					...
					postgreSTMTSelect aSel=subStmt.selectList.get(e);					
					...
					if (toMask)
						postgreParser.setMaskingConfigurationForSelect(aSel, subStmt, ddmClient);
					
					String selection_from_expr=postgreParser.getSelectionFromExpression(aSel, aSel.exprList, ddmClient, subStmt);
										
					sbSelect.append(selection_from_expr);

				
					if (aSel.alias.length()==0) {
						String generated_alias=postgreParser.generateAliasFromSelect(aSel, aSel.selection, subStmt);
						if (generated_alias.trim().length()>0) sbSelect.append(" as "+generated_alias);
					}
						
					else {
						String generated_alias=postgreParser.generateAliasFromSelect(aSel, selection_from_expr, subStmt);
						if (generated_alias.trim().length()>0) sbSelect.append(" as "+generated_alias);
					}
				...
				for (int f=0;f<subStmt.fromList.size();f++) {
					...
					if (!from.isSubQuery) {
						...
						if (postgreParser.checkTableException(from.object_owner, from.object_name, ddmClient)) continue;
				...
				if (sbWith.length()>0) {
					...
					if (ind>-1) {
						...
						sb.insert(ind, "\n");
						sb.insert(ind, postgreParser.KEYWORD_WITH);
					}
				}
				...
				if (sbSelect.length()>0) {
					...
					if (ind>-1) {
						...
						sb.insert(ind, "\n");
						sb.insert(ind, postgreParser.KEYWORD_SELECT);
					}
				}
				...
				if (sbWhere.length()>0) {
					...
					int ind=sb.indexOf(part_key);
					if (ind==-1) 
						ind=sb.indexOf("${#FROM_PART#}")+"${#FROM_PART#}".length();
					else 
						sb.delete(ind, ind+part_key.length());
					
					if (ind>-1) {
						sb.insert(ind, sbWhere.toString());
						sb.insert(ind, postgreParser.KEYWORD_WHERE);
					}
				}
				...
				if (sbFrom.length()>0) {
					...
					if (ind>-1) {
						...
						sb.insert(ind, "\n");
						sb.insert(ind, postgreParser.KEYWORD_FROM);
						sb.insert(ind, "\n ");
					}
				}

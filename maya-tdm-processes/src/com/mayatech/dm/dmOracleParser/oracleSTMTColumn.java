...
public class oracleSTMTColumn {
	...
	void compile(ddmClient ddmClient) {
		...
		if (isMasked) {
			...
			is_exception=oracleParser.checkColumnException(this, ddmClient);
			return;
		}
			...
			for (int i=0;i<sampleMaskingProfiles.size();i++) {
				...
				try {
					...
					if (rule_scope.equals("COL"))  {
						...
						ddmLib.getSampleDataOracle(this, sampleDataArr, ddmClient.dm.sample_size, ddmClient);

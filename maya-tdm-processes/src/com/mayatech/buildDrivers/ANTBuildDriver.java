...
public class ANTBuildDriver {
	...
	public ArrayList<String[]> build(ArrayList<String[]> parameters) {
		...
		mylog("****************************************************");
		mylog("***               ANT BUILD STARTED              ***");
		mylog("****************************************************");
		
		genLib.printParameters(parameters, logs);
		
		String ant_build_xml_location=genLib.getParamByName(parameters, "ANT_XML_LOCATION");
		String ant_build_prop_location=genLib.getParamByName(parameters, "ANT_PROPERTIES_LOCATION");
		
		mylog("ANT_XML_LOCATION : " + ant_build_xml_location);
		mylog("ANT_PROPERTIES_LOCATION : " + ant_build_prop_location);
		
		if (ant_build_xml_location.length()==0 || ant_build_prop_location.length()==0) {
			mylog("Build.xml or Build.Properties location is empty");
		...
		String[] copyfiles=new String[]{ant_build_xml_location,ant_build_prop_location};
